...
class SeleniumThread(threading.Thread):
    ...
    def run(self):
        if 0 == self.actionType:
            btn = CommonFunc.alarmAck(self.isMac, self.chrome, self.driver, self.host, self.port, self.username,
                                      self.password)
        elif 1 == self.actionType:
            btn = CommonFunc.alarmClear(self.isMac, self.chrome, self.driver, self.host, self.port, self.username,
                                        self.password)
        elif 2 == self.actionType:
            btn = CommonFunc.alarmQuery(self.isMac, self.chrome, self.driver, self.host, self.port, self.username,
                                        self.password)
        elif 3 == self.actionType:
            btn = CommonFunc.alarmSync(self.isMac, self.chrome, self.driver, self.host, self.port, self.username,
                                       self.password)
