...
def check_pm_all_nes():
    global nes_info
    caseutils.pre_test_case("check_nbi_pm_all_nes_cases", "nbi_pm_automation")
    for ne,info in nes_info.items():
        test_logger.logger_instance.log_info("Start to check pm data accuracy of " + info["ne_type"])     
        NBIPmFunc.PMCmpInstance(info["ne_type"],info["nename"],info["nodeid"],info["licid"],info["interval"],info["time"]).check_pm_accuracy()  
