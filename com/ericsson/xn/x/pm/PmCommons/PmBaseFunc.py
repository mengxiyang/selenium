...
def check_pm_accurate(ne_info_cfg, counter_info_cfg, server_info_path, str_end_time,
                      number_of_lic, check_rounds=None, me_counter_cfg=None):
    ...
    if check_rounds is not None:
        dict_additinal['check_rounds'] = check_rounds
    ...
    if driver:
        try:
            ...
            if 0 != len(counters_pm) % dict_additinal['number_of_lic']:
                ...
                if dict_additinal.has_key('check_rounds'):
                    rounds = dict_additinal['check_rounds']
                start_time = end_time + timedelta(minutes=-5 * rounds)
                ...
                if ne_info.has_key('tab_me') and me_counter_cfg is not None:
                    ...
                    if check_rounds is not None:
                        dict_me_add['number_of_lic'] = check_rounds
                    PmCommon.init_and_search(driver, dict_ne_info['ne_name'], end_time, start_time)
    ...
    if driver:
        try:
            ...
            if 0 != len(counters_pm) % dict_additinal['number_of_lic']:
                ...
                PmCommon.wait_until_rounds_ok(driver, dict_additinal['check_rounds'], 10, dict_additinal)
                PmCommon.check_pm_rows_updated(driver, dict_ne_info['ne_type'], counters_pm, 10, dict_additinal)
                CommonStatic.logout_rsnms(driver)
        finally:
            CommonStatic.quite_driver(driver)